...
#After running the 'shape ~ skull size + braincase ratio' model:
	...
	#Panel C - PCA with procD residuals
		...
		screen( 2 )	; par( mar = c( 4, 4, 1, 1 ) )
			plot ( PCA.resid$x [,c(X,Z)] , bty="l", pch=21, bg = ecology.bg , col="darkgrey" , cex = 2 , 
				xlab = paste0("PC",X," (",round((PCA.resid$sdev^2 / sum(PCA.resid$sdev^2))*100,2)[X], "%",")") ,
				ylab = paste0("PC",Z," (",round((PCA.resid$sdev^2 / sum(PCA.resid$sdev^2))*100,2)[Z], "%",")")		)
				#text( PCA.resid$x [,c(X,Z)] , cex = 0.5 , labels = rownames(PCA.resid$x) )
				text(PCA.resid$x [,c(X,Z)] , cex = 0.3 , labels = numbers , col = "black" )


### Write source data file
	
	source.data <- data.temp[, c("Specimen_name", "Species_ID", "Plotting_habitat", "Tree_node_number")]
		PC.values <- PCA.resid$x [,c(X,Y,Z)]
			source.data[,c("PC1.values", "PC2.values", "PC3.values")] <- PC.values
		shape.regression.skull.box.volume.scores <- size.scores	
		skull.box.volume <- procD.model$data$skull_box
			names(skull.box.volume) <- rownames(procD.model$data)
				source.data[,"shape.regression.skull.box.volume.scores"] <- shape.regression.skull.box.volume.scores
				source.data[,"skull.box.volume"] <- skull.box.volume
		shape.regression.braincase.aspect.ratio.scores <- bc.scores	
		braincase.aspect.ratio <- procD.model$data$skull_geometry
			names(braincase.aspect.ratio) <- rownames(procD.model$data)
				source.data[,"shape.regression.braincase.aspect.ratio.scores"] <- shape.regression.braincase.aspect.ratio.scores
				source.data[,"braincase.aspect.ratio"] <- braincase.aspect.ratio
				write.table(source.data, file = "Source_Data_file_2.txt")
